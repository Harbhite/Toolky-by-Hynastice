...
export default function PValueCalculator() {
  ...
  const calculatePValue = () => {
    const t = Number.parseFloat(statistic)
    const df = Number.parseFloat(degreesOfFreedom)
  ...
  const studentT = (t: number, df: number) => {
    ...
    let betaFunction = Math.exp(
      gammaLn(a) + 0.5 * Math.log(x) - gammaLn(0.5) - gammaLn(df / 2) - 0.5 * Math.log(Math.PI * df),
    )
